...
| **S5** | **问题化 + 知识点图谱**（自测问题 / 原子笔记 / 双链 / 使用指南） | ✅ 完成 |
| S6 | 学习循环 + 今日学习清单 | ⬜ |
| S7 | 视频那一半（抽帧 + 字幕段落 ↔ 画面配对） | ⬜ |
...
用户反馈：总览要的不是一张表，而是 —— ① **把知识点抽象成问题**；② **建立知识点图谱**；③ 一套平时能用的 Obsidian 学习法。

**① 问题化**（`_问题清单.md`）

陈述句读完会觉得「懂了」；**问句才会逼出「其实答不上来」**。所以让 AI 读「导览 + 该讲全部知识点（名称 / 类型 / 重要度 / 要点）」，为每个知识点出 1~3 道自测题，覆盖不同层次（为什么成立 / 怎么用 / 边界在哪 / 和什么容易混）：

```
- **[[L05.4 摩擦力：方向、静动之分与大小]]** ★★★
    - 静摩擦力的大小为何不能直接用 μsN 算出？
    - 判断摩擦力方向时为何不能看物体对地的运动方向？
    - 静摩擦与动摩擦的大小分别由什么决定？
```

出题走与 AI 制卡同一套质量闸门（空/多行/太短/太长/不是问句/空泛问法一律拒收）；批量一次出完整讲，风格一致、不重复。

**② 知识点图谱**（`知识点/*.md`）

Obsidian 的**图谱视图只认 `[[双链]]`** —— 知识点如果只是一段文本，图谱里什么都长不出来。所以：

- 每个知识点 = **一篇原子笔记**（`<id> <label>.md`，文件名自动清洗 Windows 禁用字符）
- 前置写成双链 → `[[L05.5 张力：方向、被动性与轻绳处处相等]]`
- 反向链接（「学会了它才能学」）也显式写出
- 讲次笔记 = 该讲的 MOC，知识点块标题就是双链
- 知识点被合并/改名留下的孤儿笔记会自动清掉（该目录由程序独占）

图谱里因此能看到：**枢纽**（连线多的）、**依赖链**（顺着箭头学）、**孤岛**（提取质量待查）。
